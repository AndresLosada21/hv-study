    ...
    )
{
    ...
    //
    // STEALTH: nunca revelar presenca de hypervisor.
    // O SimpleVisor original colocava o HYPERV_HYPERVISOR_PRESENT_BIT.
    // Removemos isso E qualquer signature 0x40000000.
    //
    if (VpState->VpRegs->Rax == 1)
    {
        //
        // CPUID 1: features. Limpa bit 31 de ECX (hypervisor present).
        //
        cpu_info[2] &= ~HYPERV_HYPERVISOR_PRESENT_BIT;
    }
    else if (VpState->VpRegs->Rax == HYPERV_CPUID_INTERFACE ||
             VpState->VpRegs->Rax == 0x40000000 ||
             VpState->VpRegs->Rax == 0x40000001)
    {
        //
        // Hypervisor interface detection. Devolver zeros em todos.
        //
        cpu_info[0] = 0;
        cpu_info[1] = 0;
        cpu_info[2] = 0;
    ...
    )
{
    ...
    //
    // STEALTH: Adicionado suporte pra RDMSR/WRMSR pra esconder MSRs.
    //
    ...
    case EXIT_REASON_MSR_READ:
        // STEALTH: Guest tentando ler MSR. Se for sensivel, devolve limpo.
        {
            ...
            if (ShvStealthHandleRdmsr(msr, &spoofedValue)) {
                // MSR spoofed. Devolve valor limpo ao guest.
                VpState->VpRegs->Rax = (UINT32)spoofedValue;
                VpState->VpRegs->Rdx = (UINT32)(spoofedValue >> 32);
            } else {
                // MSR nao e perigoso. Executa real.
                UINT64 realValue = __readmsr(msr);
                VpState->VpRegs->Rax = (UINT32)realValue;
                VpState->VpRegs->Rdx = (UINT32)(realValue >> 32);
            }
        }
        break;
    case EXIT_REASON_MSR_WRITE:
        ...
        {
            UINT32 msr = (UINT32)VpState->VpRegs->Rcx;
            if (!ShvStealthHandleWrmsr(msr)) {
                // MSR permitido. Executa write real.
                UINT64 value = ((UINT64)VpState->VpRegs->Rdx << 32) |
                                 (UINT64)VpState->VpRegs->Rax;
                __writemsr(msr, value);
            }
            // Se bloqueado, nao faz nada (write nao acontece)
        }
        break;
